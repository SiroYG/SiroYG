...
public class DAO {
	...
	public ArrayList<VO> getMemberList(String hotelid) {
		Connection con = dbconnect.getConnection();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			sql = "SELECT NUM, USERNAME, TITLE, TIME, HIT, INDENT from board1 where hotel_id=? order by ref desc, step asc";
			pstmt = con.prepareStatement(sql);
	...
	public void insertWrite(VO vo, int max, String hotelid) {
		Connection con = dbconnect.getConnection();
		PreparedStatement pstmt = null;
		try {
			sql = "INSERT INTO board1(NUM,USERNAME,PASSWORD,TITLE,MEMO,REF,HOTEL_ID) VALUES(BOARDNUM.nextval,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
